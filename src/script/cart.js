!function(){
    ...
    function showlist(sid, num) { //sid：编号  num：数量
        $.ajax({
            url: 'http://192.168.13.22/h5-2/zhe800/php/alldata.php',
            dataType: 'json',
        }).done(function(data) {
            alert(1)
    ...
    if ($.cookie('cookiesid') && $.cookie('cookienum')) {
        let s = $.cookie('cookiesid').split(','); //获取cookie 
        let n = $.cookie('cookienum').split(','); //获取cookie 
    ...
    function cookietoarray() {
        if ($.cookie('cookiesid') && $.cookie('cookienum')) {
            arrsid = $.cookie('cookiesid').split(','); //获取cookie 
            arrnum = $.cookie('cookienum').split(','); //获取cookie 
        } else {
            arrsid = [];
            arrnum = [];
        }
    ...
    $('.operation a').on('click', function() {
        ...
        if (window.confirm('你确定要全部删除吗?')) {
            $('.goods-item:visible').each(function() {
                if ($(this).find(':checkbox').is(':checked')) { //复选框是否选中
                    $(this).remove();
